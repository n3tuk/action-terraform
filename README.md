...
| --------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------- | ----------- |
| `action`        | <p>The action to perform through this Action, which can be one of the following: <code>validate</code>, <code>plan</code>, <code>apply</code>, <code>refresh</code>, <code>drift</code>, or <code>plan-and-apply</code>.</p>              | `true`   | `plan`      |
| `configuration` | <p>The path to the Terraform configuration to target for the specified action.</p>                                                                                                                                                        | `true`   | `terraform` |
| `module`        | <p>Set to <code>true</code> when the target configuration is a Terraform module. Module validation skips the module root and instead initialises and validates Terraform example configurations found beneath <code>examples/</code>.</p> | `false`  | `"false"`   |
